...
pub enum LocalTimelineUpdate {
    Remove(ZTenantTimelineId),
    Add(ZTenantTimelineId, Arc<DatadirTimelineImpl>),
}

impl std::fmt::Debug for LocalTimelineUpdate {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Remove(ttid) => f.debug_tuple("Remove").field(ttid).finish(),
            Self::Add(ttid, _) => f.debug_tuple("Add").field(ttid).finish(),
        }
    }
}
...
pub fn detach_timeline(
    ...
    match tenants_state::write_tenants().get_mut(&tenant_id) {
        Some(tenant) => {
            ...
            tenants_state::try_send_timeline_update(LocalTimelineUpdate::Remove(
...
fn new_local_timeline(
    ...
    tenants_state::try_send_timeline_update(LocalTimelineUpdate::Add(
